...
func (c *ComfyClient) GetSystemStats() (*SystemStats, error) {
	...
	resp, err := c.httpclient.Get(fmt.Sprintf("%s://%s/system_stats", c.serverScheme, c.serverBaseAddress))
	if err != nil {
		return nil, err
	}
...
func (c *ComfyClient) GetPromptHistoryByID() (map[string]PromptHistoryItem, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s://%s/history", c.serverScheme, c.serverBaseAddress))
	if err != nil {
		return nil, err
	}
...
func (c *ComfyClient) GetViewMetadata(folder string, file string) (string, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s://%s/view_metadata/%s?filename=%s", c.serverScheme, c.serverBaseAddress, folder, file))
	if err != nil {
		return "", err
	}
...
func (c *ComfyClient) GetImage(image_data DataOutput) (*[]byte, error) {
	...
	resp, err := c.httpclient.Get(fmt.Sprintf("%s://%s/view?%s", c.serverScheme, c.serverBaseAddress, params.Encode()))
...
func (c *ComfyClient) GetEmbeddings() ([]string, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s://%s/embeddings", c.serverScheme, c.serverBaseAddress))
	if err != nil {
		return nil, err
	}
...
func (c *ComfyClient) GetQueueExecutionInfo() (*QueueExecInfo, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s://%s/prompt", c.serverScheme, c.serverBaseAddress))
	if err != nil {
		return nil, err
	}
...
func (c *ComfyClient) GetExtensions() ([]string, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s://%s/extensions", c.serverScheme, c.serverBaseAddress))
	if err != nil {
		return nil, err
	}
...
func (c *ComfyClient) GetObjectInfos() (*graphapi.NodeObjects, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s://%s/object_info", c.serverScheme, c.serverBaseAddress))
...
func (c *ComfyClient) QueuePrompt(graph *graphapi.Graph) (*QueueItem, error) {
	...
	resp, err := c.httpclient.Post(fmt.Sprintf("%s://%s/prompt", c.serverScheme, c.serverBaseAddress), "application/json", strings.NewReader(string(data)))
...
func (c *ComfyClient) Interrupt() error {
	resp, err := c.httpclient.Post(fmt.Sprintf("%s://%s/interrupt", c.serverScheme, c.serverBaseAddress), "application/json", strings.NewReader("{}"))
	if err != nil {
		return err
	}
...
func (c *ComfyClient) EraseHistory() error {
	...
	data := "{\"clear\": \"clear\"}"
	resp, err := c.httpclient.Post(fmt.Sprintf("%s://%s/history", c.serverScheme, c.serverBaseAddress), "application/json", strings.NewReader(data))
	if err != nil {
		return err
	}
...
func (c *ComfyClient) EraseHistoryItem(promptID string) error {
	...
	resp, err := c.httpclient.Post(fmt.Sprintf("%s://%s/history", c.serverScheme, c.serverBaseAddress), "application/json", strings.NewReader(item))
	if err != nil {
		return err
	}
