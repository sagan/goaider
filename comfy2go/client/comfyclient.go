...
func NewComfyClientWithTimeout(server_scheme string, server_host string, server_port int, callbacks *ComfyClientCallbacks, timeout int, retry int) *ComfyClient {
	sbaseaddr := server_host + ":" + strconv.Itoa(server_port)
	wsScheme := "ws"
	...
	retv := &ComfyClient{
		...
		webSocket: &WebSocketConnection{
			WebSocketURL:   wsScheme + "://" + sbaseaddr + "/ws?clientId=" + cid,
...
func NewComfyClient(server_scheme string, server_host string, server_port int, callbacks *ComfyClientCallbacks) *ComfyClient {
	sbaseaddr := server_host + ":" + strconv.Itoa(server_port)
	wsScheme := "ws"
	if server_scheme == "https" {
		wsScheme = "wss"
	...
	case "executing":
		...
		if qi != nil {
			if s.Node == nil {
				...
				qi.Messages <- m
				close(qi.Messages) // @mod : close the channel since this prompt is done.
			} else {
